...
class Service(multiprocessing.Process):
    ...
    '''
    def __init__(self, ip, port, protocol, ssl, logger, ipv6=0):
    ...
    def run(self):
        ...
        while not self.exit.is_set():          
            ...
            try:
                ...
                self.log.debug("Starting service on: %s %i/%s" % (address, self.protocol))
                thread.start()
            except Exception, e:
                print e
                self.log.error(e)
...
if __name__ == "__main__":
    import utils
    logger = utils.setup_logging('catcher', './service.log', 10000, 1) 
